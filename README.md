...
В процессе каждодневной загруженной работы технической поддержки в нашей компании возникла потребность быстрее решать общие и частые вопросы. Так появилась идея — создать лёгкого помощника для общения с клиентами: передача важной информации **в один клик** в личном чате в Telegram.

# PB_SBC01_H3 как платформа для телеграм-чат-бота

> Цель проекта — развернуть на одноплатном компьютере **PB_SBC01_H3** (РВ «Полярный Медведь Электронные Технологии») постоянно работающий Telegram-бот для **общения с заказчиками**: быстрые ответы на частые вопросы, ссылки на документацию/товары, сбор обратной связи, помощь с выбором и маршрут к техподдержке.
...
* [Код бота (Python)](#код-бота-python)
...
**PB_SBC01_H3** — компактный одноплатный компьютер на SoC Allwinner H3, совместимый с Debian/Ubuntu. Он энергоэффективен, стабильно работает 24/7, имеет Ethernet/Wi-Fi и доступ к шинам GPIO/I²C/SPI/UART — удобно, если боту нужно читать датчики или управлять внешними устройствами. Это отличный «малый сервер» для Telegram-бота, который должен быть всегда онлайн.
...
* **Мгновенные ответы** по кнопкам: контакты, FAQ, ссылки на документы и разделы сайта.
...
> Это примерное древо — адаптируйте под свои задачи.
...
Создайте файл `/home/pbuser/bot/.env`:
...
* `ADMIN_USER_ID` — Telegram-ID администратора, которому бот будет пересылать отзывы.
...
> Бот использует `python-telegram-bot` v20+ (асинхронная модель).
> Логика меню ориентирована на сценарии общения с заказчиками: документы, контакты, FAQ, статус заказа, отзывы, помощь, форум, новости.
...
DOC_LINK_PASSWORD = "PBPARTY"                           # если не нужно — удалите
...
* Храните токен в `.env` с правами `0600`; владелец — пользователь бота.
* Ограничьте доступ по SSH (ключи, отключение пароля, брандмауэр).
...
* Для webhook-режима используйте HTTPS (Nginx + сертификаты). Для простоты polling обычно достаточно.
...
* Интеграция бота упростила жизнь и сотрудникам, и заказчикам: общение стало проще и быстрее.
* Удачи в создании собственного цифрового помощника!

---

### Что было исправлено

* Вынесены изображения **вне** код-блоков (они ломали подсветку и «съезжали» секции).
* Все «тройки бэктиков» **закрыты**; добавлена подсветка `bash`/`ini`/`python`.
* Оглавление приведено к корректным якорям — ссылки работают.
* Удалены лишние дефисы/текст внутри кода; заголовки отображаются правильно.
